...
var ASM_CONSTS = {
  220260: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 220311: ($0, $1, $2) => { const a = document.createElement('a'); a.style = 'display:none'; document.body.appendChild(a); var view = new Uint8Array(Module['wasmMemory'].buffer, $0, $1); var result = new Uint8Array($1); for(var i = 0; i < $1; i++) { result[i] = view[i]; } var blob = new Blob([result], { type: 'application/octet-stream' }); const url = URL.createObjectURL(blob); a.href = url; const filename = UTF8ToString($2); a.download = filename; a.click(); URL.revokeObjectURL(url); document.body.removeChild(a); },  
 220807: ($0) => { window.open(UTF8ToString($0)); },  
 220840: ($0) => { alert(UTF8ToString($0)); },  
 220867: ($0) => { document.title = UTF8ToString($0) }
};
  ...
  function ___syscall_newfstatat(dirfd, path, buf, flags) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(7, 1, dirfd, path, buf, flags);
  ...
  function ___syscall_openat(dirfd, path, flags, varargs) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(8, 1, dirfd, path, flags, varargs);
  ...
  function _environ_sizes_get(penviron_count, penviron_buf_size) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(11, 1, penviron_count, penviron_buf_size);
  ...
  function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(15, 1, fd, offset_low, offset_high, whence, newOffset);
