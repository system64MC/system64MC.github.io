...
var ASM_CONSTS = {
  238012: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 238063: ($0, $1, $2) => { const a = document.createElement('a'); a.style = 'display:none'; document.body.appendChild(a); var view = new Uint8Array(Module['wasmMemory'].buffer, $0, $1); var result = new Uint8Array($1); for(var i = 0; i < $1; i++) { result[i] = view[i]; } var blob = new Blob([result], { type: 'application/octet-stream' }); const url = URL.createObjectURL(blob); a.href = url; const filename = UTF8ToString($2); a.download = filename; a.click(); URL.revokeObjectURL(url); document.body.removeChild(a); },  
 238559: ($0) => { window.open(UTF8ToString($0)); },  
 238592: ($0) => { alert(UTF8ToString($0)); },  
 238619: ($0) => { document.title = UTF8ToString($0) }
};
  ...
  function _environ_sizes_get(penviron_count, penviron_buf_size) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(12, 1, penviron_count, penviron_buf_size);
  ...
  function _fd_fdstat_get(fd, pbuf) {
    if (ENVIRONMENT_IS_PTHREAD)
      return _emscripten_proxy_to_main_thread_js(14, 1, fd, pbuf);
...
var proxiedFunctionTable = [null,_proc_exit,exitOnMainThread,pthreadCreateProxied,___syscall_fcntl64,___syscall_fstat64,___syscall_ioctl,___syscall_lstat64,___syscall_newfstatat,___syscall_openat,___syscall_stat64,_environ_get,_environ_sizes_get,_fd_close,_fd_fdstat_get,_fd_read,_fd_seek,_fd_write];
